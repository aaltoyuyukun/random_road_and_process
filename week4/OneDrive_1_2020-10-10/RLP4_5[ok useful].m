...
RespScale = sqrt(Syy).*1000; %Response

sumL = 0; sumR = 0;

samples = 1001; % add all the wave together
for c1 = 1:samples
    
r = randi([2,1001]); %random point in scaling
f = (r/354); %frequency at the random point (0.011-2.83 cycle/m)
phase = rand*2*pi; %random phase of wave

    %Loading
aL = loadScale(r)*sqrt(f); 
%Amplitude at the frequency, loadscale is a scaling factor
wL = aL*sin(f*2*pi*t+phase); %Sine wave
sumL = sumL + wL; %Summing sine waves
end

%Response
% sys is state function, sumL is function, t is variables
